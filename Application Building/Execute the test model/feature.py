...
class FeatureExtraction:
    ...
    def __init__(self, url: object) -> object:
        self.features = []
        self.url = url
        self.domain = ""
        self.whois_response = ""
    ...
    def web_traffic(self):
        try:
            rank = BeautifulSoup(urllib.request.urlopen(
                "http://data.alexa.com/data?cli=10&dat=s&url=" + urlsafe_b64decode).read(), "xml").find("REACH")['RANK']
            if (int(rank) < 100000):
                return 1
            return 0
        except:
    ...
    def Page_Rank(self):
        try:
            prank_checker_response = requests.post(
                "https://www.checkPage_Rank.net/index.php", {"name": self.domain})
